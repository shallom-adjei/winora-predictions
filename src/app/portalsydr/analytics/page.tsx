        ...
        <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-5">
              <div className="flex items-center gap-2 mb-2"><Eye className="h-5 w-5 text-gold-400" /><p className="text-xs text-gray-400">Total Page Views</p></div>
              <p className="text-2xl font-bold">{totalViews}</p>
            </div>
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-5">
              <div className="flex items-center gap-2 mb-2"><Users className="h-5 w-5 text-gold-400" /><p className="text-xs text-gray-400">Unique Visitors</p></div>
              <p className="text-2xl font-bold">{uniqueVisitors}</p>
            </div>
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-5">
              <div className="flex items-center gap-2 mb-2"><TrendingUp className="h-5 w-5 text-gold-400" /><p className="text-xs text-gray-400">Today's Views</p></div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-6">
              ...
              <ResponsiveContainer width="100%" height={200}>
                <ReBarChart data={topPages} layout="vertical" margin={{ top: 5, right: 20, bottom: 5, left: 0 }}>
                  ...
                  <Tooltip contentStyle={{ backgroundColor: "#0D0D0D", border: "1px solid rgba(212,175,55,0.4)", borderRadius: "8px", color: "#fff" }} />
            ...
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-6">
              ...
              {deviceBreakdown.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={deviceBreakdown} cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={3} dataKey="value">
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                {deviceBreakdown.map((d, i) => (
                  <span key={d.name} className="flex items-center gap-1 text-xs text-gray-400">
                    <span className="h-3 w-3 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />{d.name}
                  </span>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-[20px] bg-[#0D0D0D] border border-white/5 p-6">
              ...
              {browserBreakdown.length > 0 ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={browserBreakdown} cx="50%" cy="50%" innerRadius={40} outerRadius={70} paddingAngle={3} dataKey="value">
                      {browserBreakdown.map((_, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]} />))}
                    </Pie>
                    <Tooltip contentStyle={{ backgroundColor: "#0D0D0D", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "8px" }} />
                  </PieChart>
                </ResponsiveContainer>
              ...
              <div className="flex flex-wrap gap-3 mt-2">
                {browserBreakdown.map((b, i) => (
                  <span key={b.name} className="flex items-center gap-1 text-xs text-gray-400">
                    <span className="h-3 w-3 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />{b.name}
                  </span>
                ))}
              </div>
            </div>
